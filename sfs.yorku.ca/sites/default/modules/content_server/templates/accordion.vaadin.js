Drupal.behaviors.cs2VaadinAccordions = {
	attach: function(context, settings){
		...
		target_accordions.on('opened-changed', function(event){
			var target_accordion = 	jQuery(this);
			var target_top = 		target_accordion.offset().top; //moved this segment inside because Safari wouldn't calculate this correctly
			
			var target_parent_panel = target_accordion.closest('.has-very-light-gray-background-color.has-background.grey-bg-block');
				...
				if(target_panel && target_parent.length){
					...
					if(target_id.length > 0){
						let scrollToTarget = function(){
							jQuery('html').scrollTop(target_id.closest('.has-background').offset().top - jQuery('.sticky-top').height());
